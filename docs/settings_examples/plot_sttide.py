...
This example tests ``ST_tide``, which chooses which published recipe sets a
star's starting rotation speed. That starting speed matters because it
feeds into how quickly a star in a binary gets tidally locked to its
companion.

``ST_tide=0`` uses the original BSE recipe (Hurley+2002). ``ST_tide=1`` uses
the StarTrack recipe (Belczynski+2008) instead, which follows a different
rule for stars above about 6.35 solar masses than for stars below it.

A set of effectively single stars is evolved across a range of ZAMS masses
under both settings. The left panel shows the starting spin assigned to
each star under each recipe. The right panel converts that spin into an
equatorial rotation speed in km/s and compares it against the published
formula for each recipe, confirming COSMIC reproduces both correctly.
"""
