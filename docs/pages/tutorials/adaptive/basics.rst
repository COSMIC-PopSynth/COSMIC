...
.. note::
    ...
    The ``merging_dco`` function requires the `LEGWORK python package <https://legwork.readthedocs.io/en/latest/>`_ to compute the merger time.  If you do not have LEGWORK installed, ``merging_dco`` will raise an error. It can be installed using ``pip install cosmic-popsynth[extras]``
