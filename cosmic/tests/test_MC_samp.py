...
class TestMC_samp(unittest2.TestCase):
    ...
    def test_sample_sech_squared(self):
        ...
        self.assertTrue(np.abs(dist_mean - SECH_SQUARED_MEAN_TEST) < 1e-4)
        dist_var = np.var(sech_squared_sample)
        self.assertTrue(np.abs(dist_var - SECH_SQUARED_VAR_TEST) < 1e-4)
    ...
    def test_sample_exponential_radial(self):
        ...
        self.assertTrue(np.abs(dist_mean - (-EXP_RADIAL_MEAN_TEST)) < 1e-4)
        dist_var = np.var(exp_radial_sample)
        self.assertTrue(np.abs(dist_var - EXP_RADIAL_VAR_TEST) < 1e-4)
    ...
    def test_sample_exponential_vertical(self):
        ...
        self.assertTrue(np.abs(dist_mean - (EXP_VERT_MEAN_TEST)) < 1e-4)
        dist_var = np.var(exp_vertical_sample)
        self.assertTrue(np.abs(dist_var - EXP_VERT_VAR_TEST) < 1e-4)
    ...
    def test_sample_exponential_square_radial(self): 
        ...
        self.assertTrue(np.abs(dist_mean - (EXP_SQUARE_RAD_MEAN_TEST)) < 1e-4)
        dist_var = np.var(exp_square_radial_sample)
        self.assertTrue(np.abs(dist_var - EXP_SQUARE_RAD_VAR_TEST) < 1e-4)
    ...
    def test_galactic_positions(self):
        ...
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2)**0.5) - MCMILLAN_THINDISK_MEAN_TEST_XY) < 1e-3)
        self.assertTrue(np.abs(np.mean(zGX) - MCMILLAN_THINDISK_MEAN_TEST_Z) < 1e-3) 
        self.assertTrue(np.abs(np.mean(inc) - 1.0) < 1e-3)
        self.assertTrue(np.abs(np.mean(OMEGA) - np.pi) < 1e-3)
        self.assertTrue(np.abs(np.mean(omega) - np.pi) < 1e-3)
       
        xGX, yGX, zGX, inc, OMEGA, omega = MC_samp.galactic_positions('ThinDisk', N_SAMP, model='sech_squared')
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2)**0.5) - SECHSQUARE_THINDISK_MEAN_TEST_XY) < 1e-3)
        self.assertTrue(np.abs(np.mean(zGX) - SECHSQUARE_THINDISK_MEAN_TEST_Z) < 1e-3)                  
        self.assertTrue(np.abs(np.mean(inc) - 1.0) < 1e-3)
        self.assertTrue(np.abs(np.mean(OMEGA) - np.pi) < 1e-3)
        self.assertTrue(np.abs(np.mean(omega) - np.pi) < 1e-3)

        xGX, yGX, zGX, inc, OMEGA, omega = MC_samp.galactic_positions('ThinDisk', N_SAMP, model='double_exp')
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2)**0.5) - DOUBLEEXP_THINDISK_MEAN_TEST_XY) < 1e-3)
        self.assertTrue(np.abs(np.mean(zGX) - DOUBLEEXP_THINDISK_MEAN_TEST_Z) < 1e-3)
        self.assertTrue(np.abs(np.mean(inc) - 1.0) < 1e-3)
        self.assertTrue(np.abs(np.mean(OMEGA) - np.pi) < 1e-3)
        self.assertTrue(np.abs(np.mean(omega) - np.pi) < 1e-3)

        xGX, yGX, zGX, inc, OMEGA, omega = MC_samp.galactic_positions('Bulge', N_SAMP, model='exp_squared')
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2+zGX**2)**0.5) - DOUBLEEXP_BULGE_MEAN_TEST_XYZ) < 1e-3)
        self.assertTrue(np.abs(np.mean(inc) - 1.0) < 1e-3)
        self.assertTrue(np.abs(np.mean(OMEGA) - np.pi) < 1e-3)
        self.assertTrue(np.abs(np.mean(omega) - np.pi) < 1e-3)

        xGX, yGX, zGX, inc, OMEGA, omega = MC_samp.galactic_positions('Bulge', 1000000, model='McMillan')
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2)**0.5) - MCMILLAN_BULGE_MEAN_TEST_XY) < 1e-3)
        self.assertTrue(np.abs(np.mean(zGX) - 0.0 < 1e-3))
        ...
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2)**0.5) - MCMILLAN_THICKDISK_MEAN_TEST_XY) < 1e-3)
        self.assertTrue(np.abs(np.mean(zGX) - MCMILLAN_THICKDISK_MEAN_TEST_Z) < 1e-3)
        self.assertTrue(np.abs(np.mean(inc) - 1.0) < 1e-3)
        self.assertTrue(np.abs(np.mean(OMEGA) - np.pi) < 1e-3)
        self.assertTrue(np.abs(np.mean(omega) - np.pi) < 1e-3)

        xGX, yGX, zGX, inc, OMEGA, omega = MC_samp.galactic_positions('ThinDisk', N_SAMP, model='double_exp')
        self.assertTrue(np.abs(np.mean((xGX**2+yGX**2)**0.5) - DOUBLEEXP_THICKDISK_MEAN_TEST_XY) < 1e-3)
        self.assertTrue(np.abs(np.mean(zGX) - DOUBLEEXP_THICKDISK_MEAN_TEST_Z) < 1e-3)
        self.assertTrue(np.abs(np.mean(inc) - 1.0) < 1e-3)
        self.assertTrue(np.abs(np.mean(OMEGA) - np.pi) < 1e-3)
        self.assertTrue(np.abs(np.mean(omega) - np.pi) < 1e-3)
