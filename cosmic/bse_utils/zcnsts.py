...
def zcnsts(z):
    ...
    *
    ...
    msp[1] = zdata.xz[5]+lzs*(zdata.xz[6]+lzs*(zdata.xz[7]+lzs*(zdata.xz[8]+lzs*zdata.xz[9])))
    msp[2] = zdata.xz[10]+lzs*(zdata.xz[11]+lzs*(zdata.xz[12]+lzs*(zdata.xz[13]+lzs*zdata.xz[14])))
    msp[3] = zdata.xz[15]+lzs*(zdata.xz[16]+lzs*(zdata.xz[17]+lzs*(zdata.xz[18]+lzs*zdata.xz[19])))
    msp[4] = zdata.xz[20]+lzs*(zdata.xz[21]+lzs*(zdata.xz[22]+lzs*(zdata.xz[23]+lzs*zdata.xz[24])))
    msp[5] = zdata.xz[25]+lzs*(zdata.xz[26]+lzs*(zdata.xz[27]+lzs*(zdata.xz[28]+lzs*zdata.xz[29])))
    msp[6] = zdata.xz[30]+lzs*(zdata.xz[31]+lzs*(zdata.xz[32]+lzs*(zdata.xz[33]+lzs*zdata.xz[34])))

    # Rzams

    msp[7] = zdata.xz[35]+lzs*(zdata.xz[36]+lzs*(zdata.xz[37]+lzs*(zdata.xz[38]+lzs*zdata.xz[39])))
    msp[8] = zdata.xz[40]+lzs*(zdata.xz[41]+lzs*(zdata.xz[42]+lzs*(zdata.xz[43]+lzs*zdata.xz[44])))
    msp[9] = zdata.xz[45]+lzs*(zdata.xz[46]+lzs*(zdata.xz[47]+lzs*(zdata.xz[48]+lzs*zdata.xz[49])))
    msp[10] = zdata.xz[50]+lzs*(zdata.xz[51]+lzs*(zdata.xz[52]+lzs*(zdata.xz[53]+lzs*zdata.xz[54])))
    msp[11] = zdata.xz[55]+lzs*(zdata.xz[56]+lzs*(zdata.xz[57]+lzs*(zdata.xz[58]+lzs*zdata.xz[59])))
    msp[12] = zdata.xz[60]
    msp[13] = zdata.xz[61]+lzs*(zdata.xz[62]+lzs*(zdata.xz[63]+lzs*(zdata.xz[64]+lzs*zdata.xz[65])))
    msp[14] = zdata.xz[66]+lzs*(zdata.xz[67]+lzs*(zdata.xz[68]+lzs*(zdata.xz[69]+lzs*zdata.xz[70])))
    msp[15] = zdata.xz[71]+lzs*(zdata.xz[72]+lzs*(zdata.xz[73]+lzs*(zdata.xz[74]+lzs*zdata.xz[75])))

    # Tbgb

    msp[16] = zdata.xt[0]+lzs*(zdata.xt[1]+lzs*(zdata.xt[2]+lzs*zdata.xt[3]))
    msp[17] = zdata.xt[4]+lzs*(zdata.xt[5]+lzs*(zdata.xt[6]+lzs*zdata.xt[7]))
    msp[18] = zdata.xt[8]+lzs*(zdata.xt[9]+lzs*(zdata.xt[10]+lzs*zdata.xt[11]))
    msp[19] = zdata.xt[12]+lzs*(zdata.xt[13]+lzs*(zdata.xt[14]+lzs*zdata.xt[15]))
    msp[20] = zdata.xt[16]

    # dTbgb/dz
    msp[116] = dlzs*(zdata.xt[1]+lzs*(2.0*zdata.xt[2]+3.0*lzs*zdata.xt[3]))
    msp[117] = dlzs*(zdata.xt[5]+lzs*(2.0*zdata.xt[6]+3.0*lzs*zdata.xt[7]))
    msp[118] = dlzs*(zdata.xt[9]+lzs*(2.0*zdata.xt[10]+3.0*lzs*zdata.xt[11]))
    msp[119] = dlzs*(zdata.xt[13]+lzs*(2.0*zdata.xt[14]+3.0*lzs*zdata.xt[15]))

    # Thook
    msp[21] = zdata.xt[17]+lzs*(zdata.xt[18]+lzs*(zdata.xt[19]+lzs*zdata.xt[20]))
    msp[22] = zdata.xt[21]
    msp[23] = zdata.xt[22]+lzs*(zdata.xt[23]+lzs*(zdata.xt[24]+lzs*zdata.xt[25]))
    msp[24] = zdata.xt[26]+lzs*(zdata.xt[27]+lzs*(zdata.xt[28]+lzs*zdata.xt[29]))
    msp[25] = zdata.xt[30]

    # Ltms
    msp[26] = zdata.xl[0]+lzs*(zdata.xl[1]+lzs*(zdata.xl[2]+lzs*(zdata.xl[3]+lzs*zdata.xl[4])))
    msp[27] = zdata.xl[5]+lzs*(zdata.xl[6]+lzs*(zdata.xl[7]+lzs*(zdata.xl[8]+lzs*zdata.xl[9])))
    msp[28] = zdata.xl[10]+lzs*(zdata.xl[11]+lzs*(zdata.xl[12]+lzs*zdata.xl[13]))
    msp[29] = zdata.xl[14]+lzs*(zdata.xl[15]+lzs*(zdata.xl[16]+lzs*(zdata.xl[17]+lzs*zdata.xl[18])))
    msp[26] = msp[26]*msp[29]
    msp[27] = msp[27]*msp[29]
    msp[30] = zdata.xl[19]+lzs*(zdata.xl[20]+lzs*(zdata.xl[21]+lzs*zdata.xl[22]))
    msp[31] = zdata.xl[23]+lzs*(zdata.xl[24]+lzs*(zdata.xl[25]+lzs*zdata.xl[26]))

    # Lalpha
    m2 = 2.0
    msp[32] = zdata.xl[27]+lzs*(zdata.xl[28]+lzs*(zdata.xl[29]+lzs*zdata.xl[30]))
    ...
    msp[52] = zdata.xr[5]+lzs*(zdata.xr[6]+lzs*(zdata.xr[7]+lzs*(zdata.xr[8]+lzs*zdata.xr[9])))
    msp[53] = zdata.xr[10]+lzs*(zdata.xr[11]+lzs*(zdata.xr[12]+lzs*(zdata.xr[13]+lzs*zdata.xr[14])))
    msp[54] = zdata.xr[15]+lzs*(zdata.xr[16]+lzs*(zdata.xr[17]+lzs*zdata.xr[18]))
    msp[55] = zdata.xr[19]+lzs*(zdata.xr[20]+lzs*(zdata.xr[21]+lzs*zdata.xr[22]))
    msp[51] = msp[51]*msp[53]
    msp[52] = msp[52]*msp[53]
    msp[56] = zdata.xr[23]
    msp[57] = zdata.xr[24]+lzs*(zdata.xr[25]+lzs*(zdata.xr[26]+lzs*zdata.xr[27]))
    msp[58] = zdata.xr[28]+lzs*(zdata.xr[29]+lzs*(zdata.xr[30]+lzs*zdata.xr[31]))
    ...
    msp[63] = (msp[56]*m2**3+msp[57]*m2**msp[60] + msp[58]*m2**(msp[60]+1.50))/(msp[59]+m2**5)

    # Ralpha
    msp[64] = zdata.xr[40]+lzs*(zdata.xr[41]+lzs*(zdata.xr[42]+lzs*zdata.xr[43]))
    msp[65] = zdata.xr[44]+lzs*(zdata.xr[45]+lzs*(zdata.xr[46]+lzs*zdata.xr[47]))
    ...
    msp[77] = zdata.xr[68]+lzs*(zdata.xr[69]+lzs*(zdata.xr[70]+lzs*zdata.xr[71]))
