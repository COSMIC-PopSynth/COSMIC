...
def error_check(BSEDict, filters=None, convergence=None):
    ...
    flag='pisn'
    if flag in BSEDict.keys():
        if not ((BSEDict[flag] > 0) or (BSEDict[flag] == -1) or (BSEDict[flag] == -2) or (BSEDict[flag] == -3) or (BSEDict[flag] == -4) or (BSEDict[flag] == -5)):
            raise ValueError("'{0:s}' needs to be set to either greater than 0 or equal to -1, -2, -3, -4, or -5 (you set it to '{1:0.2f}')".format(flag, BSEDict[flag]))
    flag='bhsigmafrac'
    if flag in BSEDict.keys():
        if (BSEDict[flag] <= 0) or (BSEDict[flag] > 1):
