...
def M_enclosed(r, gamma, rho_0):
    ...
    prefactor = 4*np.pi*rho_0*r*r*r / 3.

    hypergeometric = hyp2f1(1.5, (gamma+1.)/2., 2.5, -r**2)
...
def virial_radius_analytic(gamma, r_max):
    ...
    rho_0 = 1. / M_enclosed(r_max,gamma,1)

    work = lambda r: rho_r(r,gamma,rho_0) * r * 4 * np.pi * M_enclosed(r,gamma,rho_0)
...
def find_sigma_sqr(r, r_max_cluster, gamma):
    ...
    jeans_integrand = lambda rr:rho_r(rr,gamma,rho_0)*M_enclosed(rr,gamma,rho_0)/rr/rr 
...
def get_velocities(r,r_max_cluster,gamma):
    ...
    sigma_sqr_grid = np.array([find_sigma_sqr(rr,r_max_cluster,gamma) for rr in radii_grid])
