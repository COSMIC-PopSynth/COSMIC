...
def sample(nCore, binID, binTYPE, nBin, rangeEpoch, epochLength, SFH, maxTime, startTime, met):
    ...
    ##################################################################################
    binariesInFile = "binaries_"+binTYPE+"_"+str(nCore)+".in"
    np.savetxt(binariesInFile, initialParams, delimiter = ",")
    
    # SAVE THE FIXED POP LOG
    ##################################################################################
    fixedPopLogSave = np.vstack(fixedPopLog)
    np.savetxt("fixedPopLog_"+binTYPE+"_"+str(nCore)+".dat", fixedPopLogSave, delimiter = ',')
