...
def sample(nCore, binID, binTYPE, nBin, rangeEpoch, epochLength, SFH, maxTime, startTime, met):
    ...
    for i in rangeEpoch:
        ...
        while Nbinsave < nBin:
            '''if Nbinsave > 100:
                if kk < 1:
                    print 'at least 100 sampled', nBin
                    kk = 10
            if Nbinsave > 1000:
                if ll < 1:
                    print 'at least 1000 sampled', nBin
                    ll = 10'''
        
            # THE EVOLUTION TIME IS CONSTANT FOR EACH EPOCH OF STAR FORMATION: 
            # 'COEVAL' BINARIES
            #########################################################################
            timeConst = 0.760348964
            a_0_birth = np.random.uniform(0.0,1.0)
            tBirthsample = -7e9*np.log(1-a_0_birth*timeConst)
            tBirthsampleMyr = tBirthsample/1.0e6
            tphysf = maxTime - tBirthsampleMyr
    
            # ALL PDFs TAKEN FROM Yu&Jeffery (2015) EXCEPT THE IMF TAKEN 
            # FROM kroupa et al (1993)
            #########################################################################
            a_0 = np.random.uniform(0.0, 0.9999797)
            if a_0 <= 0.740074:
                mass_draw = ((0.1)**(-3.0/10.0)-(a_0/0.968533))**(-10.0/3.0)
            elif 0.740074 < a_0 < 0.908422:
                mass_draw = ((0.5)**(-6.0/5.0)-((a_0-0.740074)/0.129758))**(-5.0/6.0)                     
            else:
                mass_draw = (1-((a_0-0.908422)/0.0915941))**(-10.0/17.0)       
    ...
    ##################################################################################
    binariesInFile = "thinDiskTmp/lambda1/binaries_"+binTYPE+"_"+str(nCore)+".in"
    binaryFile=open(binariesInFile, "w", 0)
    np.savetxt(binaryFile, initialParams, delimiter = ",")
    binaryFile.flush()
    binaryFile.close()
    ...
    np.savetxt("thinDiskTmp/lambda1/fixedPopLog_"+binTYPE+"_"+str(nCore)+".dat", fixedPopLogSave, delimiter = ',')
